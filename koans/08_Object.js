describe("Object에 대해서 학습합니다.", function () {
  ...
  */
  ...
  it("Object를 함수의 인자로 전달할 경우, reference가 전달됩니다.", function () {
    ...
    const copiedObj = Object.assign({}, obj); // shallow copy, it's 2 different objects
    copiedObj.mastermind = "James Wood"; // copiedObj.mastermind is changed
    expect(obj.mastermind).to.equal("Joker"); // didnt changed

    obj.henchwoman = "Harley"; // is changed
    expect(copiedObj.henchwoman).to.equal("Adam West"); // didnt changed

    delete obj.twins["Jared Leto"]; // is deleted
    expect("Jared Leto" in copiedObj.twins).to.equal(false); // is not changed
