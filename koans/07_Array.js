describe("Array에 대해서 학습합니다.", function () {
  it("Array의 기본을 확인합니다.", function () {
    const emptyArr = [];
    expect(typeof emptyArr === "array").to.equal(false); // array is an object
  ...
  it("Array의 요소(element)를 다루는 방법을 확인합니다.", function () {
    ...
    expect(poppedValue).to.equal(3); // not asking the array, it's asking the "popped" one
    expect(arr).to.deep.equal([1, 2]);
  });
  ...
  it("Array 메소드 slice를 확인합니다.", function () {
    ...
    expect(arr.slice(3, 0)).to.deep.equal([]); // if start num is bigger than end, return an []
    expect(arr.slice(3, 100)).to.deep.equal(["jelly"]);
    expect(arr.slice(5, 1)).to.deep.equal([]); // same as above, start >= end
